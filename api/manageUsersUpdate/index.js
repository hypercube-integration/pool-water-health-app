// FILE: api/manageUsersUpdate/index.js
...
module.exports = async function (context, req) {
  try {
    ...
    if (!provider || !userId || !Array.isArray(roles)) return json(context, 400, { error: "BadRequest" });
    ...
    const result = await cli.staticSites.createOrUpdateStaticSiteUser(
      rg, site, provider, userId, { roles: rolesCsv }
    );

    return json(context, 200, { ok: true, result });
  } catch (e) {
    context.log.error("manageUsersUpdate:", e?.message || e);
