// Auto-upserts the current signed-in user's name/email to Cosmos using EasyAuth claims
const { CosmosClient } = require("@azure/cosmos");

module.exports = async function (context, req) {
  try {
    const cp = parseClientPrincipal(req);
    if (!cp || !isAuthenticated(cp)) return (context.res = json(401, { error: "Unauthorized" }));
...
function extractNameEmail(cp){
  let name=getClaim(cp,"name","http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name","http://schemas.microsoft.com/identity/claims/displayname")||"";
  let email=getClaim(cp,"emails","email","preferred_username","upn","http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress")||"";
