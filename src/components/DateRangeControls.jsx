...
export default function DateRangeControls({ value, onChange }) {
  ...
  const applyPreset = (id) => {
    ...
    if (id === 'ytd') { s = fmt(startOfYear()); e = fmt(today); }
    else if (id === '12m') { const d = new Date(); d.setFullYear(d.getFullYear()-1); s = fmt(d); e = fmt(today); }
    else if (id !== 'custom') { const p = PRESETS.find(p => p.id === id); if (p?.days) { s = fmt(daysAgo(p.days)); e = fmt(today); } }
    setPreset(id);
    setStartDate(s);
    setEndDate(e);
  ...
  const nativeShare = async () => {
    try { await navigator.share({ title: 'Pool Dashboard', text: 'Pool chemistry trends', url: currentUrl() }); }
    catch { /* user cancelled or not supported */ }
  };

  return (
    <div className="section date-range-controls">
      {/* Row 1: Start/End fields */}
      <div className="date-grid fields">
      ...
      {/* Row 2: Preset + Actions (Apply / Copy / Share) */}
      <div className="date-grid actions">
        <div className="date-field">
          <label>Preset</label>
        ...
        <div className="date-actions">
          ...
          {canNativeShare && <button className="secondary" onClick={nativeShare}>Share…</button>}
        </div>
      </div>
    </div>
  );
